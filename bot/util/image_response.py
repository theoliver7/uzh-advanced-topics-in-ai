...
class ImageResponseGenerator:
    ...
    def generate_image_response(self, imdb_id, is_movie=False):
        if not is_movie:
            ...
            filtered_df = self.image_df[(self.image_df['movie'].apply(lambda x: imdb_id in x)) & (self.image_df['type'] == 'poster')]
            if len(filtered_df) == 0:
                filtered_df = self.image_df[(self.image_df['movie'].apply(lambda x: imdb_id in x)) & (self.image_df['type'] == 'publicity')]
            if len(filtered_df) > 0:
                return filtered_df['img'].iloc[0].split(".jpg")[0]
            else:
